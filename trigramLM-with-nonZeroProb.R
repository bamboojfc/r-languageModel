trigrams.prob = function(firstWord, secondWord, thirdWord){
  ...
  twoWords.freq <- length(bigrams[bigrams==text2])
  threeWords.freq <- length(trigrams[trigrams==text3])
  
  prob <- threeWords.freq / twoWords.freq
  return(prob)
}
...
bigrams.prob = function(firstWord, secondWord){
  ...
  twoWords.freq <- length(bigrams[bigrams==text])
  firstWords.freq <- length(tokens[tokens==firstWord])
  prob <- twoWords.freq / firstWords.freq
...
createNGramAndCalProbs = function(d,query,column.name){
  ...
  for(s in d[,column.name]){
    ...
    u <- markov.unigrams(query.tokens)
    b <- 0
    t <- 0
    if(u!=0 && length(query.tokens) > 2){
      b <- markov.bigrams(query.tokens)
      if(b!=0 && length(query.tokens) > 3){
        t <- markov.trigrams(query.tokens)
      }
    }
    prob.query <- 0.1*u + 0.3*b + 0.6*t #back-off smoothing
